...
`valueS ~ trait, random=~vsm( dsm(trait), ism(geno) )`
...
`random= ~ vsm( usm(trait), ism(re) )`

where the `vsm` knows how to read the different variance structures for each factor. The user can specify `csm()` and provide a matrix with dimensions t x t (t being the number of traits) containing the number 0,1,2,3 that specify the constraints:
...
`random= ~ diag(env):id`     or    `random= ~ usm(env):id`

and now it would be specified as:

`fixed= y1~x`

`random= ~ vsm(dsm(env),ism(id))`    or    `random= ~ vsm(usm(env),ism(id))`

where the `dsm()` and `usm()` functions specify diagonal and unstructured models respectively. Now `csm()` for a customized structure is available. The main gain from having changed the formulation is that the new specification through the `vsm()` function allows for contructing more complex moels. For example, assume individuals specified in a column called `id` tested in three environments in a column called `env` measured at two different time points specified in a column called `time`. We may want something more flexible than:
...
`random= ~ vsm(usm(env),dsm(time),ism(id))`
...
`random= ~ usm(trait):diag(env):units` # limit

New versions (>4.4):

`random= ~ vsm(dsm(env),ism(units))` ## can be extended to more interacting factors

`random= ~ vsm(usm(env),ism(units))` ## can be extended to more interacting factors

`random= ~ vsm(atm(env),ism(units))` ## can be extended to more interacting factors

`random= ~ vsm(csm(env),ism(units))` ## can be extended to more interacting factors
...
`random=~ vsm(usm(leg(v,1)),ism(x))` 

`random=~ vsm(dsm(leg(v,1)),ism(x))` 
...
New versions (>3.7): all the power of the `mme()` function is available plus you can fit multivariate GWAS models. See details in the sommer.start vignettes.
...
New versions (>4.4): It has to be called within the `vsm()` function but now it can be combined with all the unknown covariance structures available.

`random=~ vsm(...,spl2Dc(Row,Col,at=?, thetaC=mm) )` # being mm any multi-trait constraint-structure.
...
`random=~vsm(,..., ism(Z, thetaC=mm))` # mm is any multi-trait constraint-structure.
...
This error indicates that your model is singular (phenotypic variance V matrix is not invertible) and therefore the model is stopped, throwing the error message and returning an empty list. You can try a simpler model or just modify the argument `tolparinv` in the `mme()` function. The default is 1e-3, which means that it will try to invert V and if it fails it will try to add a small value to the diagonal of V of 1e-3 to make it invertible and try bigger and biger numbers. If this fails then the program will return the empty list.
...
summary(mix1)
length(mix1$uList$`vsm(ism(id), Gu = A`) # only 100 levels
...
mix2 <- mme(Yield~1,
             ...
             data=DT2, verbose = FALSE)
summary(mix2)
length(mix2$uList$`vsm(ism(id), Gu = A`) # now 363 levels

```
As of 4.1.2 this shouldn't be a problem since internally the `mme()` solver adds the missing levels, but we leave this for reference for people using older versions of sommer.
...
data(DT_cpdata)
DT <- DT_cpdata
mixAR1row <- mme(Yield~1,
             random=~ vsm(ism(Rowf), Gu=AR1(Rowf, rho=0.3)) + Colf,
             rcov=~units,
             data=DT, verbose = FALSE)
...
data(DT_cpdata)
DT <- DT_cpdata
mixAR1col <- mme(Yield~1,
             random=~ Rowf + vsm(ism(Colf), Gu=AR1(Colf, rho=0.3)),
             rcov=~units,
             data=DT, verbose = FALSE)
...
data(DT_cpdata)
DT <- DT_cpdata
mixAR1rowcol <- mme(Yield~1,
                  random=~ vsm(ism(Rowf:Colf), 
...
When using the `vsm()` function three additional arguments help to control the following:
...
Since each random effect can be seen as a multi-trait variance covariance structure, the univariate models are just an extension where the multi-trait variance covariance structure is a 1 x 1 matrix. When inspecting the results for the mixed models fitted by the `mme()` function corresponding to the variance components stored in the `theta` element, you will notice that each random effect contains a t x t matrix which corresponds to the multi-trait structure we referred to. For example:
...
# ansm <- mme( valueS ~ trait, # henderson=TRUE,
...
mix3 <- mme(Yield~1,
              random=~vsm(ism(id, theta=initialVal, thetaC = mm), 
                          Gu=A ), # constrained
...
```{r}
mix1 <- mme(y~1, random=~vsm(usm(trait),ism(id)), data=df2, verbose = FALSE)
mix1$theta[[1]]
```
...
# now fit the model
mix2 <- mme(y~X-1, random = ~vsm(ism(sig1))+vsm(ism(sig2))+vsm(ism(gam)), data=df2, verbose = FALSE)
mix2$theta
```
...
```{r}
sig <- sig1+sig2
mix3 <- mme(y~X-1, random = ~vsm(ism(sig))+vsm(ism(gam)), data=df2, nIters=30, verbose = FALSE)
mix3$theta
```
