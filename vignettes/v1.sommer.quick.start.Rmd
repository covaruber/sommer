...
5) Details on special functions for variance models
    + the major vsm() function for special variance models and its auxiliars:
    + atm() specific levels heterogeneous variance structure
    + dsm() diagonal covariance structure
    + usm() unstructured covariance
    + csm() customized covariance structure
    + ism() function for main effects
...
## solving for c coefficients set henderson=TRUE
...
As you can see the special function `atm` or `diag` can be used to indicate that there's a different variance for the genotypes in each environment. The same was done for the residual. The difference between `atm` and `diag` is that the `atm` function can be used to specify the levels or specific environments where the variance is different.

### 3) Unstructured variance models

A more relaxed asumption than the CS+DIAG model is the unstructured model (US) which assumes that among the levels of certain factor (i.e. Environments) there's a covariance struture of a second random effect (i.e. Genotypes). This can be done in sommer using the `usm(.)` function: 
...
As can be seen the `usm(Env)` indicates that the genotypes (Name) can have a covariance structure among environments (Env).

### 4) Multivariate homogeneous variance models

Currently there's a great push for multi-response models. This is motivated by the correlation that certain variables hide and that could benefit in the prediction perspective. In sommer to specify multivariate models the response requires the use of the `cbind()` function in the response, and the `usm(trait)`, `diag(trait)`, or `atm(trait)` functions in the random part of the model. 
...
ans4c <- mme(valueS ~ Env:trait,
             random= ~ vsm(usm(trait),ism(Name)),
             rcov= ~ vsm(dsm(trait),ism(units)),
             data=DT2, verbose = F)
summary(ans4c)$varcomp
## to solve for c coefficients set henderson=TRUE

```

You may notice that we have added the `usm(trait)` behind the random effects. This is to indicate the structure that should be assumed in the multivariate model. The `diag(trait)` used behind a random effect (i.e. Name) indicates that for the traits modeled (Yield and Weight) there's no covariance component and should not be estimated, whereas `usm(trait)` assumes that for such a random effect, there's a covariance component to be estimated (i.e. covariance between Yield and Weight for the random effect Name). The same applies for the residual part (rcov).

Any number of random effects can be specified with different structures.

### 5) Details on special functions for variance models

#### the vsm() function for special variance models and its auxiliaries

The sommer function `vsm()` allows constructing complex variance models that are passed to the `mme()` function which constitutes one of the most important features of the sommer package. The specification of the `vsm()` function has the form:

`random=~vsm(..., Gu)`

The idea is that the `vsm()` function reflects  the special variance structure that each random effect could have in the matrix notation:

$var(u) = T \bigotimes E \bigotimes ... \bigotimes A$

where the ... argument in the `vsm()` function is used to specify the kronecker products from all matrices that form the variance for the random effect , where the auxiliary functions `dsm()`, `usm()`, `csm()`, `atm()`, `ism`, can be used to define such structured variance structures with intial theta values and constraints. The idea is that a variance model for a random effect x (i.e. individuals) might require a more flexible model than just:
...
would be specified in the `vsm()` function as:

`random=~vsm(usm(e),usm(s), ism(x), Gu=A)`
...
The auxiliary functions to build the variance models for the random effect are:
  ...
  + usm() unstructured covariance
  + atm() specific levels heterogeneous variance structure
  + ism() identity variance structure
  + csm() customized covariance structure
...
#### dsm() and dsm() to specify a diagonal (DIAG) covariance structures
...
`random=~vsm(dsm(e),ism(g))` 

#### usm() and usm() to specify an unstructured (US) covariance
...
`random=~vsm(usm(e),ism(g))` 

#### atm() and atc() to specify a level-specific heterogeneous variance
...
`random=~vsm(atc(e,c("A","B")),ism(g))`

where the variance component for g is only fitted at levels A and B.

#### csm() and csm() to specify a level-specific variance-covariance structure
...
`random=~vsm(csm(e,mm=c("A","B")),ism(g))`

where `mm` indicates which variance and covariance components are estimated for `g`.

### 6) The specification of multi-trait constraints in the variance components for the vsm function (Gtc argument)
...
`random= ~vsm(csm(trait, thetaC=?), ism(u))`
...
`random= ~vsm( csm(trait, thetaC=fixm(4)), ism(u) )` 

### 7) Special functions for special models

#### Random regression models

In order to fit random regression models the user can use the `leg()` function to fit Legendre polynomials. This can be combined with other special covariance structures such as `dsm() / dsm()`, `usm() / usm()`, etc.
...
In this example we assume an unreplicated population where row and range information is available which allows usm to fit a 2 dimensional spline model.
...
```
the matrix GT is provided as a random effect by encapsulating the matrix in a list and provided in the `vsm()` function.
...
mix2 <- mme(Yield~1,
            ...
            data=DT,verbose = FALSE)
```
Then to test if the second model brings value let usm fit the likelihood ratio test as follows:
...
The sommer package is equiped with a `predict()` function that can be used to calculate means and standard errors for fixed and random effects specified in the models fitted with the `mme()` and `mme` functions. Using the yatesoats dataset we will will fit some fixed and random effects.

```{r}
library(sommer)
data(DT_yatesoats)
DT <- DT_yatesoats
m3 <- mme(fixed=Y ~ V + N + V:N,
           random = ~ B + B:MP,
           rcov=~units,
           data = DT, verbose=FALSE)
summary(m3)$varcomp
