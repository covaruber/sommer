...
ansSingle <- mmer(Yield~1,
              random= ~ vsr(isr(Name), Gu=A),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansSingle)

# if setting mme=TRUE provide the inverse
# Ai <- solve(A)
...
ansDG <- mmer(Yield~Env,
              random= ~ vsr(dsr(Env),isr(Name), Gu=A),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansDG)
...
ansCS <- mmer(Yield~Env,
              random= ~ vsr(isr(Name), Gu=Ai) + vsr(isr(Env:Name), Gu=EAi),
              rcov= ~ units, 
              data=DT, verbose = FALSE)
summary(ansCS)
...
ansUS <- mmer(Yield~Env,
              random= ~ vsr(usr(Env),isr(Name), Gu=A),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansUS)
# if setting mme=TRUE provide the inverse
...
ansRR <- mmer(Yield~Env,
              random= ~ vsr(dsr(leg(EnvN,1)),isr(Name)),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansRR)
...
ansRR <- mmer(Yield~Env,
              random= ~ vsr(usr(leg(EnvN,1)),isr(Name)),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansRR)
...
ansCS <- mmer(Yield~Env,
              random= ~ vsr(isr(Name), Gu=A) + vsr(isr(Env:Name), Gu=EA),
              rcov= ~ units,
              data=DT, verbose = FALSE)
summary(ansCS)
...
mix2 <- mmer(y~ envIndex, mme=TRUE,
             random=~ Name + vsr(dsr(envIndex),isr(Name)), data=DT2,
             rcov=~vsr(dsr(Name),isr(units)),
             tolParConvNorm = .0001,
             nIters = 50, verbose = FALSE
)
# summary(mix2)$varcomp

b=mix2$uList$`vsr(dsr(envIndex), isr(Name))` # adaptability (b) or genotype slopes
mu=mix2$uList$`vsr( isr( Name ) )` # general adaptation (mu) or main effect
...
ansDG <- mmer(y~Env, mme=TRUE,
              random=~ vsr(dsr(Env), isr(Name)),
...
H0 <- ansDG$uList$`vsr(dsr(Env), isr(Name))` # GxE table

# reduced rank model
ansFA <- mmer(y~Env, mme=TRUE,
              random=~vsr( usr(rrr(Env, H = H0, nPC = 3)) , isr(Name)) + # rr
                vsr(dsr(Env), isr(Name)), # diag
              rcov=~units,
...
loadings=with(DT, rrr(Env, nPC = 3, H = H0, returnGamma = TRUE) )$Gamma
...
##########
## stage 2
## use mmer for sparse equation
##########
m <- matrix(1/var(DT2$Estimate, na.rm = TRUE))
ans2 <- mmer(Estimate~Env, mme=TRUE,
             random=~ Effect + Env:Effect, 
             rcov=~vsr(isr(units,thetaC = matrix(3), theta = m)),
             W=OM, 
             verbose=FALSE,
             data=DT2
)
