...
An eigendecomposition of a symmetric relationship matrix can be written \(A=UDU^\prime\), with orthogonal eigenvectors \(U\) and diagonal eigenvalue matrix \(D\). Premultiplying the model by \(U^\prime\) rotates the observations and design matrices into this eigenbasis, so the relationship component becomes diagonal. Under residual structures that remain diagonal after the orthogonal transformation, this can simplify marginal-covariance calculations and has historically been useful for dense direct-inversion or spectral algorithms.
...
The transformed and untransformed formulations are algebraically equivalent when the covariance structures satisfy the assumptions of the rotation. The current `mmes()` implementation, however, is no longer a dense direct-inversion algorithm: it exploits sparse mixed model equations and sparse factorization. Consequently, an eigenspace rotation is not automatically advantageous and can destroy useful sparsity in the incidence matrices. Whether such a transformation helps therefore depends on the particular model and computational formulation.
