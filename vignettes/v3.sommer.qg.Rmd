...
We know fit the indirect genetic effects model without covariance between DGE and IGE:
...
We know fit the indirect genetic effects model with covariance between DGE and IGE for which we will use the gvs() function:
...
On top of that we can a relationship matrix for the two random effects that are being forced to covary
...
As can be seen the two models are equivalent. Despite that sommer doesn't take a great advantage of this trick because it was built for dense matrices using the Armadillo library other software may be better using this trick.
