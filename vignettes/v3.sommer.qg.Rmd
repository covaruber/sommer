...
ans2 <- mmer(X1~1,
             ...
             rcov=~units, getPEV = FALSE, nIters=5, # you do more iterations
             data=y.trn, verbose = FALSE) # kinship based
...
mix <- mmer(Yield~1,
            random=~vsr(isr(id), Gu=A) +
              Rowf +
              Colf +
              spl2Dc(Row,Col), nIters=10,
            rcov=~ units,
            data=DT, verbose = FALSE)
summary(mix)
# make a plot to observe the spatial effects found by the spl2D()
W <- with(DT,spl2Dc(Row,Col)) # 2D spline incidence matrix
DT$spatial <- W$Z$`A:all`%*% mix$uList$`spl2Dc(Row, Col` # 2D spline BLUPs
