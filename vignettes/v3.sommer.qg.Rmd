...
ans.ADE <- mmer(color~1, 
                 random=~vsr(isr(id),Gu=A) + vsr(isr(idd),Gu=D), 
                 rcov=~units, nIters=10,
                 data=DT,verbose = FALSE)
...
In this example we showed how to estimate the additive ($\sigma^2_A$) and dominance ($\sigma^2_D$) variance components based on markers and estimate broad ($H^2$) and narrow-sense heritability ($h^2$). Notice that we used the `vsr()` function which indicates that the random effect inside the parenthesis (i.e. `id`, `idd` or `ide`) has a covariance matrix (A, D, or E), that will be specified in the `Gu` argument of the `vsr()` function. Please DO NOT provide the inverse, but rather the original covariance matrix when using mme=FALSE.
...
# ### fit the model
# modFD <- mmer(Yield~1, 
#               random=~ vsr(atr(Location,c("3","4")),isr(GCA2)), 
#               rcov= ~ vsr(dsr(Location),isr(units)), nIters=10,
#               returnParam = F,
#               data=DT, verbose = FALSE)
# summary(modFD)
```
...
# ### fit the model
# modFD <- mmer(Yield~1, 
#               random=~ vsr(atr(Location,c("3","4")),isr(GCA2),Gu=GT), 
#               rcov= ~ vsr(dsr(Location),isr(units)), nIters=10,
#               data=DT, verbose = FALSE)
# summary(modFD)
```
...
#### model using overlay
modh <- mmer(sugar~1, 
             random=~vsr(isr(overlay(femalef,malef)) )
             + genof, nIters=10,
             data=DT, verbose = FALSE)
...
## rrBLUP
ans2 <- mmer(X1~1,
             random=~vsr(isr(GT), buildGu = FALSE), 
             rcov=~units, getPEV = TRUE, nIters=10, # you do more iterations
             data=y.trn, verbose = FALSE) # kinship based

u <- GT %*% as.matrix(ans2$uList$`vsr(isr(GT), buildGu = FALSE`) # BLUPs for individuals
...
# modIGE <- mmer(trait ~ block, dateWarning = FALSE,
...
# ### Indirect genetic effects model
# modIGE <- mmer(trait ~ block, dateWarning = FALSE,
#                random = ~ covr( vsr(isr(focal)), vsr(isr(neighbour)) ),
...
# modIGE <- mmer(trait ~ block, dateWarning = FALSE,
#                random = ~ covr( vsr(isr(focal), Gu=Ai), vsr(isr(neighbour), Gu=Ai) ),
#                rcov = ~ units, nIters=100, verbose = FALSE,
#               data = DT)
# summary(modIGE)$varcomp
...
vv2 <- sample(vv1, 100)
y.trn[vv2,"GY"] <- NA
anss2 <- mmer(GY~1,  mme=TRUE,
              random=~vsr(isr(dent),Gu=Adi) + vsr(isr(flint),Gu=Afi), 
              rcov=~units, nIters=15,
              data=y.trn, verbose = FALSE) 
summary(anss2)$varcomp

# zu1 <- model.matrix(~dent-1,y.trn) %*% anss2$uList$`vsr(isr(dent), Gu = Adi)`
# zu2 <- model.matrix(~flint-1,y.trn) %*% anss2$uList$`vsr(isr(flint), Gu = Afi)`
...
mix.marker <- mmer(color~1,
                   random=~Rowf+vsr(isr(M)),
                   rcov=~units,data=DT, 
                   verbose = FALSE)


me.marker <- mix.marker$uList$`vsr(isr(M`
...
mix.part <- mmer(color~1,
                 random=~Rowf+vsr(isr(id), Gu=MMT),
                 rcov=~units,data=DT,
                 verbose = FALSE)

#convert BLUPs to marker effects me=M'(M'M)- u
me.part<-MTMMTinv%*%matrix(mix.part$uList$`vsr(isr(id), Gu = MMT`,ncol=1)
...
###########################
# mix2 <- GWAS(color~1,
#              random=~vsr(isr(id), Gu=A) + Rowf + Colf,
...
#               random=~vsr(isr(Z)) + Rowf + Colf,
#               rcov=~units, nIters=10,
#               verbose = FALSE,
#               data=DT)
# 
# a <- mixRRBLUP$uList$`vsr(isr(Z`# marker effects
...
#              random=~vsr(isr(id), Gu=MMT) + Rowf + Colf,
#              rcov=~units, nIters=10,
#              verbose = FALSE,
#              data=DT)
# a.from.g <-MTMMTinv%*%matrix(mixGBLUP$uList$`vsr(isr(id), Gu = MMT`,ncol=1)
...
Keep in mind that mmer uses a direct inversion (DI) algorithm which can be very slow for large datasets with many records. When datasets have more records than coefficients to be estimated please shift to the use of the mmer function.
